<template>
	<div>
		<aside class="text-right">
			<button class="s px-8 py-3 text-gray-300 font-extralight text-lg rounded-sm">
				Subscription balance: {{ fromDecimals(subBalance).toFixed(3) }} MATIC
			</button>
		</aside>
		<div class="w-full flex items-center justify-center dark:bg-gray-900">
			...
			<div
				...
				<span
					class="absolute text-md bg-black text-white font-extrabold top-0 left-0 rounded-br-lg rounded-tl-lg px-2 py-1 bg-primary-100 dark:bg-gray-900 dark:text-gray-300 border-gray-400 dark:border-gray-400 border-b-2 border-r-2 border-dashed">
					#1
				</span>
				<span
				class="absolute text-md bg-black text-white font-extrabold top-0 right-0 rounded-br-lg rounded-tl-lg px-2 py-1 bg-primary-100 dark:bg-gray-900 dark:text-gray-300 border-gray-400 dark:border-gray-400 border-b-2 border-dashed" 
				>Subscribers: {{ subscriberCount }}</span>
				...
				<div class="w-full sm:w-auto flex flex-col items-center sm:items-start">
					...
					<div class="mb-4 md:text-lg text-gray-400">
						<p>
							Stitz is a seasoned DeFi trader with over 10 years experience
							trading cryptocurrency and has amassed a total profit of over $100
							million.
						</p>
					</div>

					<div class="flex gap-4">
						<a title="youtube url" href="#" target="_blank" rel="noopener noreferrer">
					...
					<div v-if="!isOwner" class="mt-2">
						<input v-model="subscribeInputAmount" type="text" class=" p-4 rounded-sm"
							placeholder=" Greater than 0.01 MATIC" />
						<button @click="subscribe()"
							class="px-16 py-4 rounded-md mt-5 justify-center bg-orange-800 hover:bg-orange-900 text-white font-extrabold">
							SUBSCRIBE
						</button>
					</div>
					<p v-if="!isOwner" class="f font-extralight text-white text-sm">
						*Subscribing means you agree to deposit an amount
						<span class="font-extrabold">>0.01 MATIC</span> to copy all trades
						executed by this individual
					</p>
				</div>
			</div>
		</div>

		<div v-if="isOwner" class="justify-center text-center content-center">
			...
			<div class="p-1 flex flex-col items-center justify-center">
				<div class="s space-x-5 w-40 bg-orange-700 p-1 mb-4 mt-2 rounded-sm">
					<span @click="tab = 'buy'" class="font-thin cursor-pointer text-2xl"
						:class="{ 'text-white': tab == 'buy' }">Buy</span>
					<span @click="tab = 'sell'" class="font-thin cursor-pointer text-2xl vl"
						:class="{ 'text-white': tab == 'sell' }">Sell</span>
				</div>
					...
					<div v-show="tab == 'buy'">
						<div class="">
							...
							<div class="px-4 py-2 border rounded-md flex flex-col">
								<div class="flex">
									<input autocomplete="off" autocorrect="off" type="text" pattern="^[0-9]*[.,]?[0-9]*$"
										placeholder="0.0" minlength="1" maxlength="79" spellcheck="false"
										class="focus:outline-none bg-gray-500 text-gray-100" v-model="buyTokenAddress" />
						...
						<div class="">
							<div class="px-4 py-2 border rounded-md flex flex-col">
								<div class="flex justify-center">
									<input autocomplete="off" autocorrect="off" type="text" pattern="^[0-9]*[.,]?[0-9]*$"
										placeholder="0.0" minlength="1" maxlength="79" spellcheck="false"
										class="bg-gray-500 text-gray-100 focus:outline-none" v-model="buyInputAmount" />
						...
						<button @click="swapMaticToToken()"
							class="w-full bg-teal-600 hover:bg-teal-700 mt-4 rounded-md py-2 text-white">
							Swap
						</button>
					</div>
				</Transition>
						...
						<div class="">
							...
							<div class="px-4 py-2 border rounded-md flex flex-col">
								<div class="flex">
									<input autocomplete="off" autocorrect="off" type="text" pattern="^[0-9]*[.,]?[0-9]*$"
										placeholder="0.0" minlength="1" maxlength="79" spellcheck="false"
										class="focus:outline-none bg-gray-500 text-gray-100" v-model="sellTokenAddress" />
									<button class="flex bg-gray-200 rounded-sm px-4 py-1 uppercase">
										Token Address
									</button>
								</div>
						...
						<div class="">
							<div class="px-4 py-2 border rounded-md flex flex-col">
								<div class="flex justify-center">
									<input autocomplete="off" autocorrect="off" type="text" pattern="^[0-9]*[.,]?[0-9]*$"
										placeholder="0.0" minlength="1" maxlength="79" spellcheck="false"
										class="bg-gray-500 text-gray-100 focus:outline-none" v-model="sellTokenAmount" />
...
import DisperceEvents from "./DisperceEvents.vue";
...
async function checkIsOwner() {
	const contractOwner = await CTContract.owner();
	if (currentAccount.value == contractOwner) {
		isOwner.value = true;
		return;
	}
	isOwner.value = false;
...
async function getSubscriberCount() {
	const result = await CTContract.subscriberCount();
	subscriberCount.value  = result.toString()
}

const loadCurrentAccount = async () => {
	if (typeof window.ethereum !== "undefined") {
		currentAccount.value = utils.getAddress(
...
async function subscribe() {
	...
	await tx.wait(1);
	toast.success(
		"Subscribed with " + subscribeInputAmount.value + "Successfully",
...
async function swapMaticToToken() {
	...
	const tx = await CTContract.convertExactMaticToToken(buyTokenAddress.value, {
		value: msgValue,
	});
	await tx.wait(1);
	toast.success(
		"Successfully swapped to token at " +
		buyTokenAddress.value +
		" for " +
...
async function swapTokenToMatic() {
	...
	const tx = await CTContract.convertTokenToMatic(
		msgValue,
		sellTokenAddress.value
	);
	await tx.wait(1);
	toast.success(
		"Successfully sold " +
